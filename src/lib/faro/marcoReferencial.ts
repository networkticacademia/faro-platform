...
ORDEN DE USO DE FUENTES PARA CITAR — SON COMPLEMENTARIAS, NO COMPITEN ENTRE SÍ (corrección importante: NO es "usa la fuente 1 hasta agotarla, luego la 2" — cada una cumple un rol distinto y ambas deben aparecer si están disponibles):
- Corpus verificado por RSL para este proyecto → úsalo para evidencia de APLICACIÓN RECIENTE y validación empírica de que el enfoque funciona en la práctica (RSL prioriza literatura actual, no obras fundacionales antiguas — es normal y esperado que no cubra teoría clásica).
- Fuentes aportadas manualmente por el formulador (NotebookLM/Perplexity) → úsalas para la TEORÍA FUNDACIONAL/clásica que sustenta el enfoque (el "de dónde viene" el concepto) — estas suelen ser obras más antiguas que RSL no encuentra porque no prioriza antigüedad.
- Un Marco Teórico bien construido normalmente necesita AMBAS: la teoría fundacional (de las fuentes pegadas) Y evidencia de su aplicación reciente (del corpus de RSL). No dejes de citar una fuente disponible solo porque ya citaste de la otra — revisa explícitamente si hay fuentes pegadas por el formulador ANTES de dar por completo el marco_teorico.texto, y si las hay y aplican al problema, deben aparecer citadas.
- Tu propio conocimiento de entrenamiento — SOLO como último recurso, cuando ninguna de las dos fuentes anteriores cubre lo necesario, y ÚNICAMENTE con la honestidad epistémica estricta descrita abajo.
${corpusRSL ? `\nCORPUS VERIFICADO POR RSL PARA ESTE PROYECTO (rol: evidencia de aplicación reciente — fuentes ya confirmadas contra Crossref/OpenAlex/Semantic Scholar):\n"""\n${corpusRSL}\n"""\n` : ""}${fuentesExternasVerificadas ? `\nFUENTES APORTADAS MANUALMENTE POR EL FORMULADOR (rol: teoría fundacional/clásica — autor/año/DOI confirmados manualmente por él; REVÍSALAS Y ÚSALAS si aplican, no las omitas solo porque ya citaste del corpus de RSL):\n"""\n${fuentesExternasVerificadas}\n"""\n` : ""}
HONESTIDAD EPISTÉMICA ESTRICTA SOBRE CITAS DE TU PROPIO CONOCIMIENTO (cuando ni RSL ni las fuentes del formulador cubren algo) — esto es tan importante como la regla anterior: NUNCA inventes un DOI, ISBN, año exacto o título de obra que no conozcas con certeza real. Si vas a citar un autor/teoría de la que tienes conocimiento general pero no recuerdas con precisión el año exacto, el título exacto de la obra, o el DOI/ISBN, declara nivel_confianza="baja" y deja doi_o_isbn=null — NO complete esos campos con un valor inventado que parezca plausible. Es preferible una cita con nivel_confianza baja y datos incompletos honestos, que una cita completa pero fabricada — el formulador va a verificar cada referencia contra fuentes reales antes de publicar, y una cita inventada detectada destruye la confianza en todo el documento. Si citas del corpus de RSL o de las fuentes aportadas por el formulador (ambas ya verificadas), declara nivel_confianza="alta" porque ya pasaron verificación real.
