...
export function construirPromptImpactosDelimitacion(params: {
  ...
  /** Bloque de hechos ya respondidos por el formulador en iteraciones
   *  previas de ESTE mismo nodo — ver lib/faro/contextoAcumulado.ts. Sin
   *  esto, cada regeneración arranca de cero y pierde lo ya aportado. */
  hechosVerificados?: string;
}): string {
  const {
    nu, tau, subtipoDti, duracionMesesProyecto,
    rutaOutput, novaOutput, objetivosOutput, metodologiaOutput,
    feedbackIteracionAnterior, hechosVerificados,
  } = params;
